package com.mediway.hos.generator.config.builder;
...
import com.mediway.hos.generator.InjectionConfig;
import com.mediway.hos.generator.config.ConstVal;
import com.mediway.hos.generator.config.DataSourceConfig;
import com.mediway.hos.generator.config.GlobalConfig;
import com.mediway.hos.generator.config.IDbQuery;
import com.mediway.hos.generator.config.PackageConfig;
import com.mediway.hos.generator.config.StrategyConfig;
import com.mediway.hos.generator.config.TemplateConfig;
import com.mediway.hos.generator.config.po.TableField;
import com.mediway.hos.generator.config.po.TableFill;
import com.mediway.hos.generator.config.po.TableInfo;
...
public class ConfigBuilder {
    ...
    private void handlerPackage(TemplateConfig template, String outputDir, PackageConfig config) {
        ...
        packageInfo.put(ConstVal.ENTITY, joinPackage(config.getParent(),config.getModuleName(), config.getEntity()));
        packageInfo.put(ConstVal.MAPPER, joinPackage(config.getParent(), config.getModuleName(), config.getMapper()));
        packageInfo.put(ConstVal.XML, joinPackage(config.getXml(),null,  config.getModuleName()));
        packageInfo.put(ConstVal.SERVICE, joinPackage(config.getParent(), config.getModuleName(), config.getService()));
        packageInfo.put(ConstVal.SERVICE_IMPL, joinPackage(config.getParent(),config.getModuleName(),  config.getServiceImpl()));

        packageInfo.put(ConstVal.CONTROLLER, config.getParent() + StringPool.DOT + config.getModuleName() + StringPool.DOT + config.getController());
        ...
        setPathInfo(pathInfo, template.getEntity(getGlobalConfig().isKotlin()), configPathInfo.get(ConstVal.ENTITY_PATH), ConstVal.ENTITY_PATH, ConstVal.ENTITY);
        setPathInfo(pathInfo, template.getMapper(), configPathInfo.get(ConstVal.MAPPER_PATH), ConstVal.MAPPER_PATH, ConstVal.MAPPER);
        setPathInfo(pathInfo, template.getXml(), configPathInfo.get(ConstVal.XML_PATH), ConstVal.XML_PATH, ConstVal.XML);
        setPathInfo(pathInfo, template.getService(), configPathInfo.get(ConstVal.SERVICE_PATH), ConstVal.SERVICE_PATH, ConstVal.SERVICE);
        setPathInfo(pathInfo, template.getServiceImpl(), configPathInfo.get(ConstVal.SERVICE_IMPL_PATH), ConstVal.SERVICE_IMPL_PATH, ConstVal.SERVICE_IMPL);
        setPathInfo(pathInfo, template.getController(), configPathInfo.get(ConstVal.CONTROLLER_PATH), ConstVal.CONTROLLER_PATH, ConstVal.CONTROLLER);
    }
    ...
    private List<TableInfo> getTablesInfo(StrategyConfig config) {
        ...
        try {
            ...
            if (DbType.POSTGRE_SQL == dbQuery.dbType() || DbType.KINGBASE_ES == dbQuery.dbType()) {
        ...
        try {
            ...
            while (results.next()) {
                ...
                if (isId && !haveId) {
                    ...
                    if (DbType.H2 == dbQuery.dbType() || DbType.SQLITE == dbQuery.dbType()|| dbQuery.isKeyIdentity(results)) {
